...
function memberCreate() {
	...
	if(!idReg.test(joinForm.mId.value)) {
		alert("아이디는 영문, 숫자의 조합으로 4자리 이상 입력해주세요");
		joinForm.mId.focus();
		return;
	}
	...
	if (!pwReg.test(joinForm.mPw.value)) {
	...
	/* var nameReg=/^[가-힣a-zA-Z\s]+$/;
	if(!nameReg.test(joinForm.mName.value)) {
		alert("이름은 한글,영문으로만 입력가능합니다.");
		joinForm.mName.focus();
		return;
	}		 */
